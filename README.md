...
forked from https://github.com/CokeMine/EUserv_extend
...
PASSWORD: 第一个账户密码 第二个账户密码(注意是系统登陆密码，不是VPS root密码）
```
...
4、仓库内包含的 GitHub Actions 配置文件会在每隔4天，北京时间4：50，国际标准时间UTC 20：50。自动执行检查账号的脚本文件，如果检查到有需要续期的VPS，会自动续期，你也可以通过 `Push` 操作手动触发执行（建议修改脚本执行时间）。
